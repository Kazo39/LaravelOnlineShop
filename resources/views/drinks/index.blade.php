                    ...
                    <div class="card-header">
                        <div class="row">
                            ...
                            @if(auth()->user()->is_admin)
                                <div class="col-6">
                                    <a href="{{route('drink.create')}}" class="float-end  btn btn-outline-primary">Add new</a>
                                </div>
                            @endif
                        </div>
                    </div>
...
{{--                                        {{$product->name}}--}}
{{--                                    </div>--}}
{{--                                    <div class="col-2">{{$product->weight}} l</div>--}}
{{--                                    <div class="col-2">{{$product->price}} €</div>--}}
{{--                                    <div class="col-2">{{$product->description}}</div>--}}
{{--                                    <div class="col-2"><img src="{{asset('storage/'.$product->path)}}" class="img-thumbnail" alt=""></div>--}}
{{--                                    <div class="col-2 ps-5"><i class="fa-solid fa-basket-shopping fa-2xl"></i></div>--}}
{{--                                </div>--}}
{{--                            @endif--}}
{{--                        @endforeach--}}
{{--                        <div class="row mt-4">--}}
...
@section('additional_script')
    ...
    <script>
        ...
        async function getAllProducts(){
            let response = await fetch("{{route('drinks.get')}}");
            ...
            products.forEach((product) =>{
                if(product['type'] == product_type){
                    type_div += `    <div class="row mt-1 zoom">
                                        ...
                                        <div class="col-1">${product['weight']} l</div>
                                        ...
                                        <div class="col-2 ps-5 pointer" onclick="addProduct(${product['id']})" id="shop-col"><i class="fa-solid fa-basket-shopping fa-2xl"></i></div>
                                        @if(auth()->user()->is_admin)
        ...
        async function addProduct(product_id){
            let result = await fetch(`/drinks/add/${product_id}`);
        ...
        function editProduct(id){
            window.location = `/drink/${id}/edit`;
        }
        function openDeleteModal(id){
            document.getElementById('deleteForm').action = `/drink/${id}`;
