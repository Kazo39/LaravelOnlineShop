                ...
                <div class="card">
                    <div class="card-header">
                      <div class="row">
                          ...
                          <div class="col-6">
                              <a href="{{route('food.create')}}" class="float-end  btn btn-outline-primary">Add new</a>
                              <a href="{{route('addition.index', ['table' => 'food'])}}" class="btn btn-outline-secondary float-end me-3">Add new addition</a>
                          </div>
                          @endif
                      </div>
                    </div>
...
@section('additional_script')
    <script>
        async function getAllProducts(){
            let response = await fetch("{{route('food.get')}}");
        ...
        async function addProduct(product_id){
            let result = await fetch(`/food/add/${product_id}`);
            ...
            if(resultJson['status'] == true){
                ...
                window.location = `food/${product_id}`;
            }
            else{
        ...
        function editProduct(id){
            window.location = `/food/${id}/edit`;
        }

        function openDeleteModal(id){
            document.getElementById('deleteForm').action = `/food/${id}`;
        ...
        removeProductWithAdditionsAlert();
    </script>
@endsection
